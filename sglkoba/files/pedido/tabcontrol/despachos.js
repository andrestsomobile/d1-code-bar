...
var editableGridD = new EditableGrid("Despacho", {
...
EditableGrid.prototype.initializeGrid = function() 
{
	with (this) {
		...
		_$('filter').onkeyup = function() { editableGridD.filter(_$('filter').value); };
		
		// bind page size selector
		$("#pagesize").val(pageSize).change(function() { editableGridD.setPageSize($("#pagesize").val()); });
...
EditableGrid.prototype.updatePaginator = function()
{
	...
	var pages = this.getPagesInInterval(interval, function(pageIndex, isCurrent) {
		...
		return $("<a>").css("cursor", "pointer").html(pageIndex + 1).click(function(event) { editableGridD.setPageIndex(parseInt($(this).html()) - 1); });
	});
		
	// "first" link
	var link = $("<a>").html("<img src='" + image("gofirst.png") + "'/>&nbsp;");
	if (!this.canGoBack()) link.css({ opacity : 0.4, filter: "alpha(opacity=40)" });
	else link.css("cursor", "pointer").click(function(event) { editableGridD.firstPage(); });
	paginator.append(link);

	// "prev" link
	link = $("<a>").html("<img src='" + image("prev.png") + "'/>&nbsp;");
	if (!this.canGoBack()) link.css({ opacity : 0.4, filter: "alpha(opacity=40)" });
	else link.css("cursor", "pointer").click(function(event) { editableGridD.prevPage(); });
	paginator.append(link);
	...
	else link.css("cursor", "pointer").click(function(event) { editableGridD.nextPage(); });
	paginator.append(link);

	// "last" link
	link = $("<a>").html("<img src='" + image("golast.png") + "'/>&nbsp;");
	if (!this.canGoForward()) link.css({ opacity : 0.4, filter: "alpha(opacity=40)" });
	else link.css("cursor", "pointer").click(function(event) { editableGridD.lastPage(); });
	paginator.append(link);
};

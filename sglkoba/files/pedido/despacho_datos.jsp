...
<html> 
	...
	function modificarFlete(codsx) {
		var costo = document.despachoForm.despcostoflete;
		var justif = document.despachoForm.depjustflete;

		if (costo != null && justif != null) {
			
			if(confirm('Se cambiara el costo del flete, esta seguro de realizar estos cambios?')) {                     
				window.location = "despachoAction.do?opcion=setFlete&despcodsx="+codsx+"&despcostoflete="+costo.value+"&depjustflete="+justif.value;
			}
		} else {
			alert("Debe registrar un costo para el flete y justificar el cambio");
		}
	}
	
	function modificarTipoVeh(codsx) {
		var desptipoveh = document.despachoForm.desptipoveh;

		if(confirm('Se cambiara el tipo de vehículo y se recalculara el costo de flete, esta seguro de realizar estos cambios?')) {                     
				window.location = "despachoAction.do?opcion=setTipoVeh&despcodsx="+codsx+"&desptipoveh="+desptipoveh.value;
		}
	}
	</script>		
...
<table align="center" border="0" width='90%' cellpadding="3" cellspacing="0" class="tabla_datos"> 
  ...
  <tr> 
    ...
    <% if(desp_form==null) {%>  
       ...
       <%} else {%>
        <html:select property="desptipoveh" value="<%=desp_form.getDesptipoveh() %>" >
        <html:option value="TURBO">TURBO</html:option>
        <html:option value="SENCILLO">SENCILLO</html:option> 
        <html:option value="MINIMULA">MINIMULA</html:option>
        <html:option value="MULA">MULA</html:option> 
        <html:option value="VEHICULO PROPIO">VEHICULO PROPIO</html:option> 
	       ...
	       <%
	        if(desp_form!=null && desp_form.getdespestado().equals("FINALIZADO")) {   	
	       %>  
	        <a href='javascript:modificarTipoVeh(<%= desp_form.getdespcodsx()%>)'>Actualizar</a>
	       <% } %>
       <% } %>
  <tr> 
    <td>Cedula
 ...
 <tr> 
    <html:hidden property="despnotifzfp" value="0" /> 
   
    <td>Hora Pactada Cargue
    <td><html:text property="desphorapactada" value='<%= desp_form==null ? "00:00" : desp_form.getDesphorapactada() %>' /> 
    <td>Costo Flete</td>
    <td><%
        if(gstpermiso.getAutorizaciones(session, "pedido", "despacho_datos.jsp") && desp_form!=null && desp_form.getdespestado().equals("FINALIZADO")) {   	
        %> 
        <html:text property="despcostoflete" value='<%= desp_form==null ? "0.00" : desp_form.getDespcostoflete() %>' />
        <a href='javascript:modificarFlete(<%= desp_form.getdespcodsx()%>)'>Cambio Flete</a>
        <html:textarea property="depjustflete" value='<%= desp_form.getDepjustflete() %>' /> 
        <% } else { %>
        <html:text property="despcostoflete" value='<%= desp_form==null ? "0.00" : desp_form.getDespcostoflete() %>' readonly="true" styleClass="readonly" />
        <html:textarea property="depjustflete" value='<%= desp_form!=null ? desp_form.getDepjustflete() : ""%>' readonly="true" styleClass="readonly"/>
        <% } %>
    </td>
  <tr>
    <td>Guia Remesa
    <td><html:text property="depguiaremesa" />
    <td>Precinto
    <td><html:text property="depprecinto" /> 

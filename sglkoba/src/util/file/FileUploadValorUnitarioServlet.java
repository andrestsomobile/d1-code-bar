...
/**
 * This is a File Upload Servlet that is used with AJAX to monitor the progress of the uploaded file. It will return an XML object containing the meta information as well as the percent complete.
 * 
 * @author Frank T. Rios
 * 
 *         Initial Creation Date: 6/24/2007
 */
public class FileUploadValorUnitarioServlet extends HttpServlet implements Servlet {
	private static final long serialVersionUID = 2740693677625051632L;

	public FileUploadValorUnitarioServlet() {
		super();
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String tipoAdjunto = request.getParameter("tipoAdjunto");
			String ruta_pdf = request.getParameter("ruta_pdf");
			String clave = request.getParameter("clave");
			if (ruta_pdf != null && tipoAdjunto != null && clave != null) {
				filePath = filePath + ruta_pdf + File.separator + tipoAdjunto + File.separator + clave;
			} else {
				filePath = (String) s.getAttribute("ruta_upload");
			}
			...
			while (i.hasNext()) {
				...
				if (fileItem.isFormField() == false) {
					if (fileItem.getSize() > 0) {
						...
						// GstEnviarMail.enviarCorreoUploadFile(myFileName);
					}
				}
			}

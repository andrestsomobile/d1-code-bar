...
public class inventario_posicion_bodega extends Command {
	...
	private void exportar(HttpServletRequest request, reportesForm repfor) {
		...
		String comp = repfor.getCompania();
		...
		try {
			...
			hoja.mergeCells(0, renglon, 16, renglon);
				...
				if (itdet.hasNext()) {
					...
					col = 0;
					hoja.addCell(new Label(col++, renglon, "Compañia", formatT));
					...
					hoja.addCell(new Label(col++, renglon, "Categoria", formatT));
					...
					while (itdet.hasNext()) {
						...
						String compa =  (String) datadet.next();
						...
						String categor =  (String) datadet.next();
						...
						hoja.addCell(new Label(col++, renglon, compa, format));
						...
						hoja.addCell(new Label(col++, renglon, categor, format));
	...
	public class reporte extends GstTabla {
		...
		public Collection detalle (String  comp)  {
			...
			String consulta = "select comnombre,bodnombre,posicionbodega, promodelo, prodescripcion, proump, ump_estiba, catnombre, string_agg(distinct trafnumdta::text,',') as trafnumdta,  \r\n" + 
					...
					" from (\r\n" + 
					" \r\n" + 
					"select comnombre,cbbodega as bod, cbestante||cbposicion as posicionbodega, promodelo, prodescripcion, proump, catnombre, trafnumdta, round(prounimasterpac,0) as ump_estiba, entcodsx,entposicion,\r\n" + 
					" Round(entsaldonac,0) as saldonac, Round(entsaldosinnac,0) as saldosnac, ltraflote, ltrafvencimiento,\r\n" + 
					"case when cbestado = 'IN' then 'TUNEL' else (case when cbentrada is not null then 'OCUPADA' else 'LIBRE' end) end as tunel,cbentrada,cbestante, cbnivel, cbposicion\r\n" + 
					"from config_bodega \r\n" + 
					"left join entrada on entcodsx = cbentrada\r\n" + 
					"left join producto on procodsx = entcodproducto \r\n" + 
					"left join categoria on catcodsx = procategoria \r\n" +
					"left join compania on comcodsx = procodigocia \r\n" + 
					"left join lote_trafico on ltrafcodsx = entlote\r\n" + 
					"left join trafico on trafcodsx = ltrafnumtrafico\r\n" + 
					"where cbbodega != 1\r\n" + 
					"and (comcodsx is null or comcodsx = " + comp + ")\r\n" + 
					//"where cbentrada is not null and (entsaldonac+ entsaldosinnac) > 0\r\n" + 
					
					"union all\r\n" + 
					"select comnombre,entbodega as bod,entposicion as posicionbodega, promodelo, prodescripcion, proump, catnombre, trafnumdta, round(prounimasterpac,0) as ump_estiba, entcodsx, entposicion,\r\n" + 
					...
					"group by comnombre,bod,bodnombre, posicionbodega, promodelo, prodescripcion,proump,ump_estiba,catnombre,cbestante, cbnivel, cbposicion,entposicion\r\n" + 
					"\r\n" + 
					"order by \r\n" + 

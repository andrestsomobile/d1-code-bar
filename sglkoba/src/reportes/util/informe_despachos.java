...
public class informe_despachos extends Command {
	...
	private void exportar(HttpServletRequest request, reportesForm repfor) {
		...
		try {
			...
			if (grupo != null && grupo.equals("7")) {
				registros = rep.informe_despachos("dn_"+ usu.getusucodsx() , repfor.getFecha_ini(), repfor.getFecha_fin());
			} else {
				registros = rep.informe_despachos(repfor.getFecha_ini(), repfor.getFecha_fin());
	...
	public class reporte extends GstTabla {
		...
		public Collection informe_despachos (String usuario, String fecha_ini, String fecha_fin)  {
			...
			String consulta = "select distinct \r\n" + 
					...
					"inner join config C on C.cfnombre = '" + usuario + "' and string_to_array(s.succodigo::text,',') && string_to_array(C.cfvalor,',')\r\n" + 

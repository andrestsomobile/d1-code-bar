...
var editableGridCarga = new EditableGrid("PedidosCarga", {
...
EditableGrid.prototype.initializeGrid = function() 
{
	with (this) {
		...
		_$('filter').onkeyup = function() { editableGridCarga.filter(_$('filter').value); };
		
		// bind page size selector
		$("#pagesize").val(pageSize).change(function() { editableGridCarga.setPageSize($("#pagesize").val()); });
	}
};
...
EditableGrid.prototype.updatePaginator = function()
{
	...
	var pages = this.getPagesInInterval(interval, function(pageIndex, isCurrent) {
		...
		return $("<a>").css("cursor", "pointer").html(pageIndex + 1).click(function(event) { editableGridCarga.setPageIndex(parseInt($(this).html()) - 1); });
	});
		
	// "first" link
	var link = $("<a>").html("<img src='" + image("gofirst.png") + "'/>&nbsp;");
	if (!this.canGoBack()) link.css({ opacity : 0.4, filter: "alpha(opacity=40)" });
	else link.css("cursor", "pointer").click(function(event) { editableGridCarga.firstPage(); });
	paginator.append(link);

	// "prev" link
	link = $("<a>").html("<img src='" + image("prev.png") + "'/>&nbsp;");
	if (!this.canGoBack()) link.css({ opacity : 0.4, filter: "alpha(opacity=40)" });
	else link.css("cursor", "pointer").click(function(event) { editableGridCarga.prevPage(); });
	paginator.append(link);
	...
	else link.css("cursor", "pointer").click(function(event) { editableGridCarga.nextPage(); });
	paginator.append(link);

	// "last" link
	link = $("<a>").html("<img src='" + image("golast.png") + "'/>&nbsp;");
	if (!this.canGoForward()) link.css({ opacity : 0.4, filter: "alpha(opacity=40)" });
	else link.css("cursor", "pointer").click(function(event) { editableGridCarga.lastPage(); });
	paginator.append(link);
};

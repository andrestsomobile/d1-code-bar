...
<table align="center" border="0">
	<tr>
		<td  width="35%"><img src="./disenno/images/logo.gif" width="120" height="120">
		<td width="50%" align="center"><b> PLANILLA DE DESPACHOS <br>
		<%=emp.getempnombre()%> <br>
		<%=emp.getempdireccion()%><br>
		NIT <%=emp.getempnit()%> </b>
		<td width="15%"><%=desp.getdespfecha()%>
</table>
...
Collection pedidos = gdesp.getListaPlanillaDespacho(despcodsx);
				...
				while (ite.hasNext()) {
					...
					//se cambia kilovolumen por estibas
					kilovol = gdesp.getEstibasDespachoPedido(despcodsx,kilovol);
					peso =  util.Math.sumar(peso, util.Math.multiplicar("20",kilovol) );
		...
		out.println("Salida");
		       //if(pedidos.size()==0) pedidos = gdesp.getListaPlanillaDespachonew(despcodsx);
				...
				while (ite.hasNext()) {
					...
					//se cambia kilovolumen por estibas
					kilovol = gdesp.getEstibasDespachoPedidoGuia(despcodsx,kilovol);
					peso =  util.Math.sumar(peso, util.Math.multiplicar("20",kilovol) );
					vtotal = util.Math.sumar_bigdecimal(vtotal.toPlainString(),val);
					vtotalfac = util.Math.sumar_bigdecimal(vtotalfac.toPlainString(),valfac);
					ptotal = util.Math.sumar_bigdecimal(ptotal.toPlainString(),peso);
		...
		out.println("Salida");
		       //if(pedidos.size()==0) pedidos = gdesp.getListaPlanillaDespachonew(despcodsx);
				...
				while (ite.hasNext()) {
					Collection datos = (Collection) ite.next();
					Iterator data = datos.iterator();
					String succodsx = (String) data.next();
					String succodigo = (String) data.next();
					Collection fs = gped.getlistapedidoByClienteByDespacho(desp.getdespempresa(), succodsx, despcodsx);
					String seriales = "";
					String factura = "";
					String pe = "";
					String orden = "";
					...
					kilovol = gdesp.getEstibasDespachoPedidoCliente(despcodsx,kilovol);
					peso =  util.Math.sumar(peso, util.Math.multiplicar("20",kilovol) );
					vtotal = util.Math.sumar_bigdecimal(vtotal.toPlainString(),val);
					vtotalfac = util.Math.sumar_bigdecimal(vtotalfac.toPlainString(),valfac);
					ptotal = util.Math.sumar_bigdecimal(ptotal.toPlainString(),peso);

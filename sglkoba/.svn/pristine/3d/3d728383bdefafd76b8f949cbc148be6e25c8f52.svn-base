<%@ page contentType="text/html; charset=iso-8859-1"
	language="java"  
	import="java.util.*,reportes.util.*,reportes.form.*, administracion.entity.*"
%>
<%@taglib prefix="html" uri="/WEB-INF/struts-html.tld" %> 

<% 
informe_despachos informe = new informe_despachos();
informe_despachos.reporte reporte = informe.new reporte ();
...
if (fecha_ini != null && fecha_fin  != null){
	...
	if (grupo != null && grupo.equals("7")) {
		data = reporte.informe_despachos("dn_"+ usu.getusucodsx() , fecha_ini, fecha_fin);
	} else {
		data = reporte.informe_despachos(fecha_ini, fecha_fin);
	}
...
<html>
		 ...
		 function validar(){
			 ...
			 //respuesta
			 	if(mens!='') {
			 		...
			 		document.reportesForm.pagina.value = 'main.jsp?app=reportes&modulo=informe_despachos&tipo=operaciones&fecha_ini='+document.reportesForm.fecha_ini.value+'&fecha_fin='+document.reportesForm.fecha_fin.value;	
			 		return true;
			 }
		</script>	
	...
	<body>
		<header>
			<br />
			<div class="subtitulo">INFORME DESPACHOS <%=grupo != null && grupo.equals("7") ? "-REGIONALES" : "" %></div>
		</header>
		...
		if(fecha_ini == null || fecha_fin == null) {		
		%>
		<html:form action="reportesAction.do?tipo=operaciones&reporte=informe_despachos&tipo_reporte=EXCELGENERADO" method="post" onsubmit="return validar()">
			<input type="hidden" name="pagina" value="" />
			<input type="hidden" name="app" value="reportes"/>
			<input type="hidden" name="modulo" value="informe_despachos"/>

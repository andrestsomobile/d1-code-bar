...
<% 
informe_pedidos_pendientes informe = new informe_pedidos_pendientes();
informe_pedidos_pendientes.reporte reporte = informe.new reporte ();
...
if (fecha_ini != null && fecha_fin  != null){
	...
	data = reporte.informe(fecha_ini, fecha_fin);
...
<html>
		 ...
		 function validar(){
			 ...
			 //respuesta
			 	if(mens!='') {
			 		...
			 		document.reportesForm.pagina.value = 'main.jsp?app=reportes&modulo=informe_pedidos_pendientes&tipo=operaciones&fecha_ini='+document.reportesForm.fecha_ini.value+'&fecha_fin='+document.reportesForm.fecha_fin.value;	
			 		return true;
			 }
		</script>	
	...
	<body>
		<header>
			<br />
			<div class="subtitulo">INFORME PEDIDOS PENDIENTES DE DESPACHO</div>
		</header>
		...
		if(fecha_ini == null || fecha_fin == null) {		
		%>
		<html:form action="reportesAction.do?tipo=operaciones&reporte=informe_pedidos_pendientes&tipo_reporte=EXCELGENERADO" method="post" onsubmit="return validar()">
			<input type="hidden" name="pagina" value="" />
			<input type="hidden" name="app" value="reportes"/>
			<input type="hidden" name="modulo" value="informe_pedidos_pendientes"/>

...
Collection pedidos = gdesp.getListaPlanillaDespacho(despcodsx);
				...
				while (ite.hasNext()) {
					...
					kilovol = gdesp.getKiloVolumenDespachoPedido(despcodsx,
							kilovol);
					vtotal = util.Math.sumar_bigdecimal(vtotal.toPlainString(),
							val);
					ptotal = util.Math.sumar_bigdecimal(ptotal.toPlainString(),
							peso);
					btotal = util.Math.sumar_bigdecimal(btotal.toPlainString(),
							bultos);
					ktotal = util.Math.sumar_bigdecimal(ktotal.toPlainString(),
							kilovol);
	...
	<tr>
		...
		<td align="right"><%=util.JhFormat.getFormatedNumber(vtotal
								.toPlainString())%>
		<td align="right"><%=util.JhFormat.getFormatedNumber(ptotal
								.toPlainString())%>
		<td align="right"><%=util.JhFormat.getFormatedNumber(btotal
								.toPlainString())%>
		<td align="right"><%=util.JhFormat.getFormatedNumber(ktotal
								.toPlainString())%>
</table>

...
<% 
movimientos_calidad informe = new movimientos_calidad();
movimientos_calidad.reporte reporte = informe.new reporte ();
...
<html>
		 ...
		 function validar(){
			 ...
			 //respuesta
			 	if(mens!='') {
			 		...
			 		document.reportesForm.pagina.value = 'main.jsp?app=reportes&modulo=movimientos_calidad&tipo=operaciones&fecha_ini='+document.reportesForm.fecha_ini.value+'&fecha_fin='+document.reportesForm.fecha_fin.value;	
			 		return true;
			 }
		</script>	
	...
	<body>
		<header>
			<br />
			<div class="subtitulo">MOVIMIENTOS BODEGA CALIDAD</div>
		</header>
		...
		if(fecha_ini == null || fecha_fin == null) {		
		%>
		<html:form action="reportesAction.do?tipo=operaciones&reporte=movimientos_calidad&tipo_reporte=EXCELGENERADO" method="post" onsubmit="return validar()">
			<input type="hidden" name="pagina" value="" />
			<input type="hidden" name="app" value="reportes"/>
			<input type="hidden" name="modulo" value="movimientos_calidad"/>

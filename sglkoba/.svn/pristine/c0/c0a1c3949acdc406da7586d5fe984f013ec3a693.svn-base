...
function crearEntradasParciales(codsx,ingcod) {
	if(confirm('Se crearan las entradas para este ingreso parcial del lote, esta seguro de realizar estos cambios?')) 
		window.location = "ingresoAction.do?opcion=crearEntradasParciales&ingcodsx="+ingcod+"&lctrafcodsx="+codsx;
}
</script>
<%
				...
				<tr align='center'>
					...
					<td><%if (temp_det.getlctrafingreso() != null && temp_det.getlctrafingreso().equals("N")) {%>
						<form name="miforma" action="">
			          		<a href='javascript:crearEntradasParciales(<%= temp_det.getlctrafcodsx()%>,<%=ingf.getingcodsx()%>)'> 
					        Crear entradas</a>
			           </form>
					   <%} %>		    
